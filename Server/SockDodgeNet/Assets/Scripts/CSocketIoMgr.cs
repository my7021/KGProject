...
namespace socketionet
{
    ...
    public partial class CSocketIoMgr : MonoBehaviour
    {
        ...
        public event EventHandler<SORoomUserState> onAck_RoomUserReady = null;          // 룸에서 준비상태값 응답
        public event EventHandler<SORoomChangeMaster> onAck_RoomChangeMaster = null;    // 방장 변경
        ...
        public async void InitSocketIO(string sIP, int nPort )
        {
            ...
            socket.OnUnityThread("notify_room_ready", (res) =>
            {
                ...
                if (onAck_RoomUserReady != null)
                    onAck_RoomUserReady(this, kState);
            });
            // 방장(마스터) 변경    
            socket.OnUnityThread("notify_room_change_master", (res) =>
            {
                ...
                if (onAck_RoomChangeMaster != null)
                    onAck_RoomChangeMaster(this, kMaster);
